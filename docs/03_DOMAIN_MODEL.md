...
- **Company:** tenant root owning memberships, teams, jobs, reports, and attachments. Future business entities should also be company-scoped.
...
`Item` is a company-owned cataloged or tracked thing: consumable material, stock, equipment, serialized device, or another resource. Tracking behavior—quantity-based, serialized, consumable, durable—must be explicit. It may have a company-unique custom ID, unit, serial/lot identity, category, bundle membership, assignment, movement, or installation location.

`ItemCategory` is company-owned classification. It must not substitute for typed tracking behavior. Later asset/vehicle models should reuse common resource identity where appropriate instead of duplicating it.
...
├── ItemCategory ── Item ── ItemMovement
